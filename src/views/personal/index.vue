      ...
      <div v-if="activeTab === 'sharing'" class="sharing-view">
        <div class="sharing-header">
          ...
          <div class="sharing-actions">
            <el-button @click="refreshSharingData">
        ...
        <el-tabs v-model="sharingActiveTab" class="sharing-tabs">
          <!-- 我的分享标签页 -->
          <el-tab-pane label="我的分享" name="my-shares">
            <div class="shares-filter">
              <el-select v-model="shareTypeFilter" placeholder="分享类型" clearable @change="filterShares">
              ...
              <el-select v-model="shareStatusFilter" placeholder="分享状态" clearable @change="filterShares">
            ...
            <div class="sharing-list-container" v-loading="sharingLoading">
              <el-table :data="filteredShares" style="width: 100%">
                <el-table-column label="内容名称" min-width="200">
        ...
        <div class="pagination-container">
          ...
          <!-- 共享给我标签页 -->
          <el-tab-pane label="共享给我" name="shared-with-me">
            <div v-if="sharedWithMeList.length === 0" class="shared-with-me-empty">
              <el-empty description="暂无共享内容" />
            </div>
            <div v-else>
              <el-table :data="sharedWithMeList" style="width: 100%">
                <el-table-column label="内容名称" min-width="200">
                  <template #default="scope">
                    <div class="share-name">
                      <el-icon :size="18" class="share-icon">
                ...
                <el-table-column label="操作" width="150">
                  <template #default="scope">
                    <div class="share-actions">
                      ...
                      <el-button size="small" text @click="addToFavorites(scope.row)">
...
const openDocument = (doc: any) => {
  if (doc.isFolder) {
    // 如果是文件夹，进入该文件夹
    currentPath.value.push({ id: doc.id, name: doc.name })
    console.log('进入文件夹:', doc.name)
    // 重置分页
    currentPage.value = 1
  } else {
    // 如果是文件，预览该文件
...
const searchDocuments = () => {
  console.log('搜索文档:', searchKeyword.value)
  currentPage.value = 1
  // 实际应用中应调用API搜索文档
}
...
const handleRowClick = (row: any) => {
  if (row.isFolder) {
    // 如果是文件夹，进入该文件夹
    currentPath.value.push({ id: row.id, name: row.name })
...
const createNewFolder = () => {
  ElMessageBox.prompt('请输入文件夹名称', '新建文件夹', {
    ...
    if (value.trim()) {
      // 创建新文件夹对象
      const newFolderId = Math.max(...documents.value.map(doc => doc.id)) + 1
...
const sharedWithMeList = ref([
  {
    ...
    type: 'document',
    sharedBy: '张三',
    sharedAt: new Date('2023-11-25T09:30:00'),
    expiryDate: new Date('2024-12-31T23:59:59'),
    views: 12,
    isPublic: false,
    link: 'https://example.com/share/s001'
  },
  {
    id: 's002',
    title: '产品设计资源合集',
    type: 'folder',
    sharedBy: '李四',
    sharedAt: new Date('2023-11-20T14:15:00'),
    expiryDate: null, // 永久有效
    views: 8,
    isPublic: true,
    link: 'https://example.com/share/s002'
  },
  {
    id: 's003',
    ...
    type: 'learning-plan',
    sharedBy: '王五',
    sharedAt: new Date('2023-11-15T11:20:00'),
    expiryDate: new Date('2023-12-15T23:59:59'), // 已过期
    views: 5,
    isPublic: false,
    link: 'https://example.com/share/s003'
  },
  {
    id: 's004',
    title: '后端技术知识图谱',
    type: 'knowledge-map',
    sharedBy: '赵六',
    sharedAt: new Date('2023-11-10T16:45:00'),
    expiryDate: new Date('2024-05-10T23:59:59'),
    views: 20,
    isPublic: true,
    link: 'https://example.com/share/s004'
  }
]);
